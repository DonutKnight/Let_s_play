...
class MovingSaw(pygame.sprite.Sprite):
    def __init__(self, obj):
        ...
        self.axis, self.dist, self.speed, self.anim_speed, self.direction, self.frame_index = obj.properties.get('axis', 'y'), float(obj.properties.get('dist', 100)), float(obj.properties.get('speed', 2.0)), float(obj.properties.get('anim_speed', 0.2)), 1, 0.0
...
class FallingStone(pygame.sprite.Sprite):
    def __init__(self, obj):
        ...
        self.activated_by, self.instant_death, self.on_impact = obj.properties.get('activated_by', ''), obj.properties.get('instant_death', True), obj.properties.get('on_impact', 'stop')
...
class MovingPlatform(pygame.sprite.Sprite):
    def __init__(self, obj):
        ...
        self.axis, self.dist, self.speed = obj.properties.get('axis', 'x'), obj.properties.get('dist', 100), obj.properties.get('speed', 1.0)
    ...
    def update(self, *args, **kwargs):
        ...
        if self.axis == 'x':
            ...
            self.pos_vec.y += self.speed * self.direction
            if abs(self.pos_vec.y - self.start_pos.y) >= abs(self.dist): self.direction *= -1
        self.rect.topleft, self.vel = (round(self.pos_vec.x), round(self.pos_vec.y)), pygame.math.Vector2(self.rect.x - old_x, self.rect.y - old_y)


class Player(pygame.sprite.Sprite):
    def __init__(self, pos, character_name="Ninja Frog"):
        super().__init__()
        self.character_name = character_name
        # Ładowanie odpowiednich klatek dla wybranej postaci
        try:
            raw_img = pygame.image.load(f"assets/Main Characters/{character_name}/Idle (32x32).png").convert_alpha()
            self.idle_img = raw_img.subsurface((0, 0, 32, 32))
            raw_dj = pygame.image.load(f"assets/Main Characters/{character_name}/Double Jump (32x32).png").convert_alpha()
            self.dj_frames = [raw_dj.subsurface((i * 32, 0, 32, 32)) for i in range(6)]
        except (pygame.error, FileNotFoundError):
            self.idle_img = pygame.Surface((32, 32))
            self.idle_img.fill((0, 255, 0))
            self.dj_frames = [self.idle_img] * 6
...
def load_level(level_module: Any, spawn_name: str = "PlayerSpawn"):
    ...
    player.position, player.rect.topleft, player.velocity = pygame.math.Vector2(found_pos or (100, 100)), (int(found_pos[0]), int(found_pos[1])) if found_pos else (100, 100), pygame.math.Vector2(0, 0)
        ...
        if isinstance(layer, pytmx.TiledTileLayer):
            ...
            for obj in layer:
                ...
                if "wyjscie" in str(obj.name).lower() or obj_tp == "exit":
                    exits.append({'rect': pygame.Rect(int(obj.x), int(obj.y), int(obj.width), int(obj.height)), 'name': str(obj.name)})
...
def main_menu(screen):
    ...
    for char in characters:
        try:
            img = pygame.image.load(f"assets/Main Characters/{char}/Idle (32x32).png").convert_alpha()
            char_images.append(pygame.transform.scale(img.subsurface((0, 0, 32, 32)), (64, 64)))
        except (pygame.error, FileNotFoundError):
            surf = pygame.Surface((64, 64))
            surf.fill((255, 255, 255))
            char_images.append(surf)

    try:
        bg_image = pygame.image.load("assets/Background/Blue.png").convert()
    except (pygame.error, FileNotFoundError):
    ...
    inst_font = pygame.font.SysFont("Arial", 16, bold=True)
    ...
    while running:
        ...
        title = menu_font.render("WYBIERZ POSTAĆ", True, (255, 255, 255))
        screen.blit(title, (SCREEN_WIDTH // 2 - title.get_width() // 2, 40))

        start_x = SCREEN_WIDTH // 2 - (len(characters) * 80) // 2 + 8
        for i, img in enumerate(char_images):
            cx = start_x + i * 80
            cy = 130
            rect = screen.blit(img, (cx, cy))

            if i == selected:
                pygame.draw.rect(screen, (255, 215, 0), rect, 3)
                name_txt = inst_font.render(characters[i], True, (255, 215, 0))
                screen.blit(name_txt, (SCREEN_WIDTH // 2 - name_txt.get_width() // 2, 220))

        instruction = inst_font.render("Strzałki: Wybór | ENTER/SPACJA: Start", True, (200, 200, 200))
...
if __name__ == "__main__":
    ...
    selected_character = main_menu(screen)

    clock = pygame.time.Clock()
    player = Player((0, 0), selected_character)
    load_level(lewelOne)
