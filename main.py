        ...
        if isinstance(layer, pytmx.TiledTileLayer):
            ...
            for obj in layer:
                ...
                if "wyjscie" in str(obj.name).lower() or obj_tp == "exit":
                    ...
                    moving_platforms.add(MovingPlatform(obj));
                    group.add(moving_platforms.sprites()[-1])
                elif obj_tp == "movingsaw" or obj.name == "saw":
                    moving_saws.add(MovingSaw(obj));
                    group.add(moving_saws.sprites()[-1])
                elif obj_tp == "fallingstone":
                    falling_stones.add(FallingStone(obj));
                    group.add(falling_stones.sprites()[-1])
                elif obj.name == "BossAnchor":
                    iron_heads.add(IronHead(obj));
                    group.add(iron_heads.sprites()[-1])
                elif obj.name == "BossSpawn":
                    boss_group.add(Boss(obj));
                    group.add(boss_group.sprites()[-1])
                elif obj.name == "SpikedHead" or obj_tp == "surprise":
                    surprise_traps.add(SurpriseSpikeHead(obj));
                    group.add(surprise_traps.sprites()[-1])
                elif obj_tp == "fruit":
                    fruits_group.add(Fruit(obj));
                    group.add(fruits_group.sprites()[-1])
                elif obj_tp == "trigger" or "Trigger" in str(obj.name):
                    triggers.append({
                        'rect': pygame.Rect(int(obj.x), int(obj.y), int(obj.width), int(obj.height)),
                        'name': str(obj.name),
...
def main_menu(screen):
    ...
    # Ładowanie grafik postaci do podglądu
    ...
    while running:
        ...
        # Pozycjonowanie obrazków na środku
        ...
        for i, img in enumerate(char_images):
            ...
            # Podświetlenie i etykieta jeśli wybrany
            if i == selected:
...
# Wyświetlanie menu głównego przed startem
selected_character = main_menu(screen)

clock = pygame.time.Clock()
# Inicjalizacja gracza z nowo wybraną postacią
        ...
        if ev.type == pygame.KEYDOWN:
            if show_results:
                if ev.key in [pygame.K_SPACE, pygame.K_RETURN]:
                    ...
                    # Po przejściu gry ekran wraca do menu wyboru postaci
                    selected_character = main_menu(screen)
                    player = Player((0, 0), selected_character)
